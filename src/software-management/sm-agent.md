...
In case of declaring a capability for a child device, the child's device Id must be added as sub-topic to the request (e.g. `tedge/capabilities/software/update/<device Id>`).
...
In case the request is addressed to a child device, the `deviceId` reflects it's child's device Id. Otherwise it's value is an empty string ("").
...
* `deviceId` reflects the child's device Id in case the according request was addressed to a child device. Otherwise the value is an empty string ("").
